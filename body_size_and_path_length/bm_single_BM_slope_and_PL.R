...
data$log_pl_mam = data$mammal*data$log_pl
#########################################################################
...
cross_validate_seperate_slopes<-function(taxa_remove){
  ...
  mod1<-MCMCglmm(cbind(neo,mal) ~ trait+trait:mammal+trait:log_rec + trait:log_bm+ trait:log_pl + trait:log_pl_mam, family=cbind("poisson","poisson"), pr=TRUE,prior = pr1, data=data, pl=TRUE, 
  ...
  saveRDS(mod1,file=paste0("~/results/n_1_single_bm_seperate_pl_",taxa_remove,".rds"))
}
...
for (i in 1:length(species_list)){
  mod1<-readRDS(paste0("~/results/n_1_single_bm_seperate_pl_",species_list[i],".rds"))
...
write.csv(neoplasia_outlier_results, '~/results/bm_neoplasia_single_bm_seperate_pl_studentized_residuals.csv',row.names=FALSE)
write.csv(malignant_outlier_results, '~/results/bm_malignant_single_bm_seperate_pl_studentized_residuals.csv',row.names=FALSE)
#########################################################################
...
mod1<-MCMCglmm(cbind(neo,mal) ~ trait+trait:mammal+trait:log_rec + trait:log_bm + trait:log_pl + trait:log_pl_mam, family=cbind("poisson","poisson"), pr=TRUE,prior = pr1, data=filtered_data, pl=TRUE, 
...
saveRDS(mod1,'~/results/single_bm_seperate_pl.rds')

tmp<-as.data.frame(mod1$Sol[,c(1:12)])
class_vec<-rep(c("Aves","Mammalia"),each=nrow(tmp)*2)
growth<-rep(rep(c("neo","mal"),each=nrow(tmp)),2)
neo_aves<-cbind(tmp[,1],tmp[,5],tmp[,7],tmp[,9])
mal_aves<-cbind(tmp[,1]+tmp[,2],tmp[,6],tmp[,8],tmp[,10])
neo_mammals<-cbind(tmp[,1]+tmp[,3],tmp[,5],tmp[,7],tmp[,9]+tmp[,11])
...
write.csv(output,'~/results/single_bm_seperate_pl_parameter_estimates.csv',row.names=FALSE)
#########################################################################
#Single PL slope
...
cross_validate_single_slope<-function(taxa_remove){
  ...
  mod1<-MCMCglmm(cbind(neo,mal) ~ trait+trait:mammal+trait:log_rec + trait:log_bm+ trait:log_pl, family=cbind("poisson","poisson"), pr=TRUE,prior = pr1, data=data, pl=TRUE, 
                 rcov = ~ us(trait):units, random = ~us(trait):Species,nitt=number_iterations,thin=sample_interval,burnin=burnin_iterations, verbose=FALSE, DIC=TRUE, ginverse=list(Species=treeAinv))
  saveRDS(mod1,file=paste0("~/results/n_1_single_bm_single_pl_",taxa_remove,".rds"))
}
...
malignant_outlier_results<-array(data=NA,c(0,2))
for (i in 1:length(species_list)){
  mod1<-readRDS(paste0("~/results/n_1_single_bm_single_pl_",species_list[i],".rds"))
  pred<-as.data.frame(predict.MCMCglmm(mod1,data,interval="prediction",type="terms",marginal = NULL))
...
write.csv(neoplasia_outlier_results, '~/results/bm_neoplasia_single_bm_single_pl_studentized_residuals.csv',row.names=FALSE)
write.csv(malignant_outlier_results, '~/results/bm_malignant_single_bm_single_pl_studentized_residuals.csv',row.names=FALSE)
#########################################################################
neoplasia_sig_outlier_results<-neoplasia_outlier_results[neoplasia_outlier_results$neo_abs_res_val > 3,]
malignant_sig_outlier_results<-malignant_outlier_results[malignant_outlier_results$mal_abs_res_val > 3,]
...
mod1<-MCMCglmm(cbind(neo,mal) ~ trait+trait:mammal+trait:log_rec + trait:log_bm + trait:log_pl, family=cbind("poisson","poisson"), pr=TRUE,prior = pr1, data=filtered_data, pl=TRUE, 
               rcov = ~ us(trait):units, random = ~us(trait):Species,nitt=number_iterations,thin=sample_interval,burnin=burnin_iterations, verbose=TRUE, DIC=TRUE, ginverse=list(Species=treeAinv))

saveRDS(mod1,'~/results/single_bm_single_pl.rds')
...
colnames(output)<-c("class","growth","intercept","records","bm_slope","pl_slope")
output[3:6]<-lapply(output[3:6],as.numeric)
write.csv(output,'~/results/single_bm_single_pl_parameter_estimates.csv',row.names=FALSE)
#########################################################################
pr1<- list(R=list(V=diag(2), nu=2, fix=2),B=list(mu=(rep(0,4)),V=diag(4)*1e8),
           G=list(G1=list(V=diag(2), nu=2, alpha.mu=cbind(0,0), alpha.V=diag(2)*25^2)))

null_mod<-MCMCglmm(cbind(neo,mal) ~ trait+trait:mammal, family=cbind("poisson","poisson"), pr=TRUE,prior = pr1, data=filtered_data, pl=TRUE, 
                   rcov = ~ us(trait):units, random = ~us(trait):Species,nitt=number_iterations,thin=sample_interval,burnin=burnin_iterations, verbose=TRUE, DIC=TRUE, ginverse=list(Species=treeAinv))

saveRDS(null_mod,'~/results/bm_intercept.rds')
